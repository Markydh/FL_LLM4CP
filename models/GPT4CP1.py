...
class Model(nn.Module):
    ...
    def __init__(self, gpt_type='gpt2', d_ff=768, d_model=768, gpt_layers=6,
        ...
        # 指定本地模型路径
        local_model_path = 'Weights/gpt2'  # 请替换为你的实际路径

        # 根据 gpt_type 从本地加载不同版本的 GPT-2 模型
        if gpt_type == 'gpt2-medium':
            self.gpt2 = GPT2Model.from_pretrained(os.path.join(local_model_path, 'gpt2-medium'), 
                                                  output_attentions=True, output_hidden_states=True)
            self.gpt2.h = self.gpt2.h[:gpt_layers]
            self.gpt_dim = 1024
        elif gpt_type == 'gpt2-large':
            self.gpt2 = GPT2Model.from_pretrained(os.path.join(local_model_path, 'gpt2-large'), 
                                                  output_attentions=True, output_hidden_states=True)
            self.gpt2.h = self.gpt2.h[:gpt_layers]
            self.gpt_dim = 1280
        elif gpt_type == 'gpt2-xl':
            self.gpt2 = GPT2Model.from_pretrained(os.path.join(local_model_path, 'gpt2-xl'), 
                                                  output_attentions=True, output_hidden_states=True)
            self.gpt2.h = self.gpt2.h[:gpt_layers]
            self.gpt_dim = 1600
        else:
            self.gpt2 = GPT2Model.from_pretrained(local_model_path,  # 默认加载 gpt2
                                                  output_attentions=True, output_hidden_states=True)
            self.gpt2.h = self.gpt2.h[:gpt_layers]
            self.gpt_dim = 768

        # 微调模型 层归一化和位置编码梯度会改变， mlp为1的多层感知机也会改变
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        x_enc_r = rearrange(x_enc, 'b l (k o) -> b l k o', o=2)         # 分离实部和虚部
        x_enc_complex = torch.complex(x_enc_r[:, :, :, 0], x_enc_r[:, :, :, 1])  #  创建复数张量
        x_enc_delay = torch.fft.ifft(x_enc_complex, dim=2)         # 根据复数矩阵 进行逆傅立叶变换 生成时域的复数矩阵
        ...
        x_enc = x_enc_fre + x_enc_delay     # 时域+频域
        x_enc = rearrange(x_enc, 'b o l k -> b l (k o)', o=2)  # [B, L, D]

        enc_out = self.enc_embedding1(x_enc, x_mark_enc)  # [B, L, 768] (1024, 16, 768)
